...
upperBound = np.array([102, 235, 255])
...
while True:
    ...
    if(len(contours) == 2):
        ...
        cv2.rectangle(img, (x1, y1), (x1+y1), (w1+h1), (255, 0, 0), 2)
        cv2.rectangle(img, (x2, y2), (x2+y2), (w2+h2), (255, 0, 0), 2)
        cx1 = x1 + w1 / 2
        cy1 = y1 + h1 / 2
        cx2 = x2 + w2 / 2
        cy2 = y2 + h2 / 2
        ...
        if(pinchFlag ==0):
            if(abs((w * h - openx * openh)*100 / (w * h)) < 30):
                pinchFlag = 1
